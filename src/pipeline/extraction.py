...
from constants.settings import DEFAULT_GEMINI_MODEL, DEFAULT_VERCEL_MODEL
...
class StructuredExtractionSupport:
    @staticmethod
...
class GeminiStructuredExtractor:
    """Vision extractor backed directly by Google AI Studio.
    ...
    def extract_record(
        ...
        image_specs = StructuredExtractionSupport._record_image_paths(
            record,
            self.config.tracker_root,
        )
        image_paths = StructuredExtractionSupport._image_paths_for_item(
            record.item_id,
            self.config.tracker_root,
        )
        prompt = StructuredExtractionSupport.build_prompt(record.item_type)
        raw_response = self._call_api(image_specs, prompt)
        raw_payload, parse_error = StructuredExtractionSupport._parse_json_object(
            raw_response
        )
        normalized_payload = StructuredExtractionSupport.normalize_payload(
            record.item_type, raw_payload
        )
        filter_report = StructuredExtractionSupport.build_filter_report(
            record.item_type,
            raw_payload,
            normalized_payload,
        )
...
@dataclass(slots=True)
class VercelGatewayExtractorConfig:
    model_id: str = DEFAULT_VERCEL_MODEL
    api_key: str | None = None
    tracker_root: str | None = None
    timeout_seconds: float = 180.0


class VercelGatewayStructuredExtractor:
    """Vision extractor using Vercel AI Gateway's OpenAI-compatible API."""

    def __init__(self, config: VercelGatewayExtractorConfig) -> None:
        self.config = config
        self.model_id = config.model_id
    ...
    def _call_api(
        ...
        api_key = self.config.api_key or os.environ.get("AI_GATEWAY_API_KEY", "")
        if not api_key:
            raise RuntimeError(
                "AI_GATEWAY_API_KEY is not set. "
                "Either pass --api-key or export AI_GATEWAY_API_KEY."
            )
            ...
            content.append(
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:{_get_mime_type(image_path)};base64,{encoded}"
                    },
                }
            )
        ...
        for attempt in range(max_retries + 1):
            ...
            if response.status_code < 400:
                ...
                if not choices:
                    raise RuntimeError("Vercel AI Gateway returned no choices")
                message = choices[0].get("message") or {}
                content_value = message.get("content")
                if not isinstance(content_value, str):
                    raise RuntimeError("Vercel AI Gateway returned a non-text response")
            ...
            try:
                ...
                delay = 2.0 * (2**attempt)
            print(
                f"  [Gateway retry {response.status_code}] "
